...
from venom.models_mitino import (
    MitinoSeo, MitinoNews, MitinoPromo,
    MitinoGallery, PhotoMitino, MitinoZonesNew
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # --- SEO ---
        seo_old = MitinoSeo.objects.last()
        if seo_old:
        ...
        # --- Новости ---
        for item in MitinoNews.objects.all():
        ...
        self.stdout.write(f"✅ Новости: {MitinoNews.objects.count()} шт.")

        # --- Акции ---
        for item in MitinoPromo.objects.all():
        ...
        self.stdout.write(f"✅ Акции: {MitinoPromo.objects.count()} шт.")

        # --- Галерея ---
        for g in MitinoGallery.objects.all():
        ...
        self.stdout.write(f"✅ Галерея: {MitinoGallery.objects.count()} шт.")

        # --- Фото клуба ---
        for p in PhotoMitino.objects.all():
        ...
        self.stdout.write(f"✅ Фото верхней галереи: {PhotoMitino.objects.count()} шт.")

        # --- Игровые зоны ---
        for z in MitinoZonesNew.objects.all():
        ...
        self.stdout.write(f"✅ Зоны: {MitinoZonesNew.objects.count()} шт.")
